...
const GuestManagement: React.FC<GuestManagementProps> = ({ allLogs, onViewGuestHistory, currentUser, onLogout, onAddNewGuest, onEditGuest }) => {
  ...
  const filteredGuests = useMemo(() => {
    if (!deferredQuery.trim()) return uniqueGuests;
    const lower = deferredQuery.toLowerCase();
    return uniqueGuests.filter((item) =>
      `${item.guest.firstName} ${item.guest.lastName}`.toLowerCase().includes(lower) ||
      item.guest.phoneNumber.includes(lower) ||
      item.guest.email.toLowerCase().includes(lower),
    );
  }, [uniqueGuests, deferredQuery]);
